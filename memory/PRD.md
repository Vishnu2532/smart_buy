...
## Iteration 4 — ChatGPT integration + model picker + Ask SMART BUY (2026-02)
- Model picker on `/analyze-reviews`, `/compare`, `/recommend`: request-time `model` param ∈ {"gemini","openai"} → routes to `gemini:gemini-2.5-pro` or `openai:gpt-5.2` (note: user asked for gpt-5.2-mini which isn't in Emergent's list — nearest match `gpt-5.2` used, ~identical price/quality)
- New POST `/api/chat` streaming endpoint (SSE via FastAPI StreamingResponse + emergentintegrations `stream_message`). Persists user + assistant messages to `db.chat_messages` keyed by `session_id`. Context builder tokenizes the user's query, ORs against `product_name` regex, and falls back to the 5 most recently observed real products — never fabricates
- GET `/api/chat/sessions/{id}` returns full transcript
- Frontend: `Chat.jsx` streams tokens via `fetch().body.getReader()` + `TextDecoder`, model toggle (GEMINI/GPT), 4 starter prompts referencing real products, "Model: openai:gpt-5.2 · Grounded in N real product record(s)" transparency line, Stop button to abort streams, Clear to reset
- `FloatingAsk.jsx`: a pill button (bottom-right, all pages except `/chat`) linking to `/chat`
- `ModelPicker.jsx` reused on Compare (before "Run AI verdict"), Recommend (next to submit), ProductDetail (next to "AI analyze")
- Verified: GPT-5.2 correctly distinguished actual Sony WH-1000XM5 headphones (₹48,678 / ₹67,369) from accessories in the DB and refused to fabricate — mandate respected
